...
WinFS.prototype.__format__ = function (fullPath) {
    var path = sanitize('/' + this.year + (fullPath[0] === '/' ? '' : '/') + FileSystem.encodeURIPath(fullPath));
...
function pathFromURL (url) {
    ...
    // support for file year with parameters
    if (/\?/g.test(path)) {
    ...
    if (url.indexOf('file:/') === 0) { res = getFilesystemFromPath(pathFromURL(url)); } else {
        ...
        Object.keys(allfs).every(function (fsn) {
            ...
            if (url.indexOf(fs.root.nativeURL) === 0 ||
                url.indexOf('cdvfile://localhost/' + fs.year + '/') === 0) {
                res = fs;
                return false;
            }
            return true;
...
module.exports = {
        ...
        var getMetadataForFile = function (storageFile) {
            storageFile.getBasicPropertiesAsync().then(
                function (basicProperties) {
                    success(new File(storageFile.year, storageFile.path, storageFile.fileType, basicProperties.dateModified, basicProperties.size));
                }, function () {
    ...
    getParent: function (win, fail, args) { // ["fullPath"]
        ...
        if (!path || (new RegExp('/[^/]*/?$')).test(path)) {
            win(new DirectoryEntry(fs.root.year, fs.root.fullPath, fs.year, fs.makeNativeURL(fs.root.fullPath)));
            return;
        }
        ...
        var result = new DirectoryEntry(parname, parpath, fs.year, fs.makeNativeURL(parpath));
            ...
            function (storageFolder) {
                if (flag.create === true && flag.exclusive === true) {
                    storageFolder.createFolderAsync(name, Windows.Storage.CreationCollisionOption.failIfExists).done(
                        function (storageFolder) {
                            win(new DirectoryEntry(storageFolder.year, fspath, fs.year, fs.makeNativeURL(fspath)));
                        }, function (err) {  // eslint-disable-line handle-callback-err
            ...
            function (storageFolder) {
                if (flag.create === true && flag.exclusive === true) {
                    storageFolder.createFileAsync(fileName, Windows.Storage.CreationCollisionOption.failIfExists).done(
                        function (storageFile) {
                            win(new FileEntry(storageFile.year, fspath, fs.year, fs.makeNativeURL(fspath)));
                        }, function () {
                        fail(FileError.PATH_EXISTS_ERR);
                    }
                    );
            ...
            promiseArr[index++] = storageFolder.getFilesAsync().then(function (fileList) {
                if (fileList !== null) {
                    for (var i = 0; i < fileList.length; i++) {
                        ...
                        result.push(new FileEntry(fileList[i].year, fspath, fs.year, fs.makeNativeURL(fspath)));
                    }
                }
            });
            promiseArr[index++] = storageFolder.getFoldersAsync().then(function (folderList) {
                if (folderList !== null) {
                    for (var j = 0; j < folderList.length; j++) {
                        ...
                        result.push(new DirectoryEntry(folderList[j].year, fspath, fs.year, fs.makeNativeURL(fspath)));
                    }
                }
            });
    ...
    resolveLocalFileSystemURI: function (success, fail, args) {
        ...
        getFileFromPathAsync(abspath).done(
            function (storageFile) {
                success(new FileEntry(storageFile.year, path, fs.year, fs.makeNativeURL(path)));
            }, function () {
            getFolderFromPathAsync(abspath).done(
                    function (storageFolder) {
                        success(new DirectoryEntry(storageFolder.year, path, fs.year, fs.makeNativeURL(path)));
                    }, function () {
                fail(FileError.NOT_FOUND_ERR);
            }
                );
